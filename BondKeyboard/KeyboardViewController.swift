...
class KeyboardViewController: UIInputViewController {
    ...
    var currentWordBuffer: String = ""

    override func viewDidLoad() {
        super.viewDidLoad()
        print("The custom keyboard has been loaded")
        self.addKeyboardButtons()
    ...
    override func viewDidDisappear(_ animated: Bool) {
        ...
        print("The custom keyboard has been disappeared")
    }
    override func updateViewConstraints() {
        super.updateViewConstraints()
        // Add custom view sizing constraints here
    }
    func checkKeyboardActivity() {
            if let contextBeforeInput = textDocumentProxy.documentContextBeforeInput,
               !contextBeforeInput.isEmpty {
                // There is text before the cursor, meaning the keyboard is actively being used
                print("The keyboard is in use. Text is being typed.")
            } else {
                // No text before the cursor, so the keyboard is not in use
                print("The keyboard is not in use.")
            }
        }
    ...
    override func textDidChange(_ textInput: UITextInput?) {
       ...
       // self.setColorScheme(colorScheme)
        ...
        // Safely unwrap returnKeyType and returnButton
        if let returnKeyType = self.textDocumentProxy.returnKeyType,
           let titleType = UIReturnKeyType(rawValue: returnKeyType.rawValue) {
            
            let returnTitle = titleType.get(rawValue: titleType.rawValue)

            if let returnButton = self.returnButton {
                returnButton.setTitle(returnTitle, for: .normal)
            } else {
                print("⚠️ Warning: returnButton is nil in textDidChange.")
            }
        } else {
            print("⚠️ Warning: returnKeyType or titleType is nil.")
        }
    }
    ...
    func setNextKeyboardVisible(_ visible: Bool) {
        guard let nextKeyboardButton = self.nextKeyboardButton else {
            print("⚠️ Warning: nextKeyboardButton is nil in setNextKeyboardVisible.")
            return
        }
...
//        secondRowView.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
    ...
    func serveiceKeys(midRow: UIView)->(UIStackView, UIStackView) {
        ...
        fourthRowSV.axis = .horizontal
        fourthRowSV.distribution = .fill
        fourthRowSV.spacing = 10

        // Add width constraints
        let totalWidth = UIScreen.main.bounds.width - 4 // you can also use `view.frame.width` if inside viewDidAppear

        // 25% width for numericButton
        self.numericButton.widthAnchor.constraint(equalToConstant: totalWidth * 0.25).isActive = true

        // 50% width for spaceKey
        spaceKey.widthAnchor.constraint(equalToConstant: totalWidth * 0.50).isActive = true

        // 25% width for returnButton
        self.returnButton.widthAnchor.constraint(equalToConstant: totalWidth * 0.25).isActive = true
    ...
    @objc func didTapButton(sender: UIButton) {
        ...
        // Insert the tapped character
        proxy.insertText(title)
        
        // If the key is a letter or emoji, add to buffer
        let punctuationSet = CharacterSet(charactersIn: " .,!?")
        if title.rangeOfCharacter(from: punctuationSet) != nil || title == "return" || title == "space" || title == "\n" {
            // Word ended: print and clear buffer
            if !currentWordBuffer.isEmpty {
                print("Completed word: \(currentWordBuffer)")
                // You can also send this word to your backend here
                
                currentWordBuffer = ""
            }
        } else {
            // Append the typed character to buffer
            currentWordBuffer += title
        }
    }
    ...
    @objc func insertWhiteSpace() {
        ...
        proxy.insertText(" ")
        print("white space")
        print ("word text ", currentWordBuffer)
    }
